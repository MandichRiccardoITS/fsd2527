...
function uploadFilesFromCommit() {
    ...
    for file in $changedFiles; do
        ...
        local ftpRequest="ftp://fsd2527:PasswordCalendario2527@ftp.fsd2527.altervista.org:21/$relativePath/$fileName"
        ...
        if [ $curlExitCode -ne 0 ]; then
            ...
            if [[ "$curlOutput" == *"Failed to open/read local data"* ]]; then
                ...
                local deleteCommand="curl -Q \"DELE $relativePath/$fileName\" \"ftp://fsd2527:PasswordCalendario2527@ftp.fsd2527.altervista.org:21/\" --ftp-pasv"
